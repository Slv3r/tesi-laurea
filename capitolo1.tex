...
CONNECTION\_ REQ: inviata da un dispositivo in stato di scanning, consente a quest'ultimo di effettuare una connessione con il dispositivo in stato di Advertising che ha dichiarato la sua disponibilità a connettersi.

\begin{figure}[H]
\includegraphics[width=150pt]{conn_req_payload}
\centering
\caption{Payload di un PDU di tipo CONNECTION\_ REQ }
...
\centering
\caption{Campi di una struttura LLData di un pacchetto PDU di tipo CONNECTION\_ REQ}
\label{lldata}
\end{figure}
...
\item WinSize: determina il valore di ${transmitWindowSize = WinSize \cdot 1.25\ ms}$
\item WinOffset
\item Interval
\item Latency indica il numero di volte che ???
\item Timeout 
...
\subsection{Stato di connessione}
Il Link Layer entra in uno stato di connessione quando l'initiator invia un pacchetto di tipo CONNECTION\_ REQ (Capitolo \ref{conn_req}); dopo aver ricevuto il pacchetto la connessione viene considerata creata ma non stabilita. Dopo che viene inviato il primo pacchetto dati la connessione si considera stabilita.
Durante una connessione i due dispositivi assumono due ruoli ben distinti:
\begin{itemize}
\item Chi ha inviato il pacchetto di CONNECTION\_ REQ assumerà il ruolo di \emph{Master} in posizione di dispositivo Centrale
...
\subsubsection{Supervision Timeout}
...
Se in fase di connessione questo timer raggiunge il valore di $6*connInterval$ prima che la connessione sia stabilita, essa si considera fallita.
A connessione stabilita, il parametro connection supervisor timeout (\emph{connSupervisionTimeout}) definisce il tempo massimo che deve passare tra la ricezione di due PDU di tipo data successive; Deve essere un multiplo di 10 ms con un valore compreso tra 10 ms e 32 s calcolato come $(1 + connSlaveLatency) * connInterval$.
...
In figura \ref{transmit_window} è mostrato un evento di connessione a livello Link Layer, in cui sono visualizzati sia i pacchetti di Advertise con la relativa CONNECTION\_ REQ, sia i primi pacchetti di dati scambiati. Il dispositivo Master può settare l'Anchor Point a suo piacimento, per gestire eventuali altre connessioni a cui partecipa; l'Anchor Point viene settato quando il Master invia il primo pacchetto dati nella transmitWindow. Essa inizia a 1,25 ms + \emph{transmitWindowOffset} dopo aver ricevuto il PDU della CONNECTION\_ REQ; il primo pacchetto non deve essere inviato più tardi di 1.25 ms + transmitWindowOffset + transmitWindowSize.
